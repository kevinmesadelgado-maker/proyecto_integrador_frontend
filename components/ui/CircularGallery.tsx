...
function createTextTexture(
  gl,
  text,
  font = "600 38px 'Figtree', sans-serif",
  color = "white"
) {
  const canvas = document.createElement("canvas");
  const ctx = canvas.getContext("2d");

  // --- Primer cálculo para tamaño ---
  ctx.font = font;
  const metrics = ctx.measureText(text);
  const textWidth = Math.ceil(metrics.width);
  const textHeight = Math.ceil(parseInt(font, 10) * 1.2);

  // --- Medidas finales con más Padding ---
  canvas.width = textWidth + 70;
  canvas.height = textHeight + 55;

  ctx.font = font;
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";

  // === Fondo elegante más llamativo, con borde redondeado ===
  const radius = 24; // 🔵 Borde redondeado
  ...
  // 🖤 Fondo oscuro translúcido elegante
  ctx.fillStyle = "rgba(0,0,0,0.60)";
  ctx.fill();

  // ✨ Glow sutil para que destaque más
