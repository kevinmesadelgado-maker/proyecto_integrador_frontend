...
export default function ArtistasPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const message =
          err instanceof Error ? err.message : "Error al cargar artistas";
        setError(message);
      } finally {
        setLoading(false);
      }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-900">
        <h1 className="text-xl text-gray-300">
          Cargando el directorio de artistas...
        </h1>
      </div>
    );
  }
  ...
  const galleryItems = artistasData.map((artist) => ({
    image:
      artist.image ||
      "https://via.placeholder.com/800x600?text=Artista+Sin+Imagen", // URL de fallback
    text: artist.name,
  }));
